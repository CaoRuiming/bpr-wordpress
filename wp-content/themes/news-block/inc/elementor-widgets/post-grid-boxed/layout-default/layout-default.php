<?php
/**
 * Post Grid Boxed block layout default - php render.
 */
...
?>
        <?php
            ...
            if( !empty( $postCategory ) && ( $posttype == 'post' ) ) {
                $grid_boxed_post_args['category_name'] = implode( ',', $postCategory );
            } elseif( !empty( $postCategory ) ) {
                $grid_boxed_post_args['tax_query'] = array(
                    array( 'taxonomy' => esc_html( $taxonomy_name ),
                            'terms' => array( implode( ',', $postCategory ) )
                ));
            }

            $grid_boxed_post_query = new WP_Query( $grid_boxed_post_args );
            if( ! $grid_boxed_post_query->have_posts() ) {
                esc_html_e( 'No posts found', 'news-block' );
            }
            $total_posts = $grid_boxed_post_query->post_count;
            while( $grid_boxed_post_query->have_posts() ) : $grid_boxed_post_query->the_post();
                $postid = get_the_ID();
                ...
                $current_post = $grid_boxed_post_query->current_post;
                if( ( $current_post % 6 ) === 0 ) {
                    echo '<div class="grid-boxed--left-block the_stickey_class">';
                } else if ( ( $current_post % 6 ) === 2 ) {
                    echo '<div class="grid-boxed--main-block the_stickey_class">';
                } else if ( ( $current_post % 6 ) === 3 ) {
                    echo '<div class="grid-boxed--right-block the_stickey_class">';
                }
        ?>
        ...
        <?php
            if( ( $current_post % 6 ) === 1 ) {
                echo '</div><!-- .grid-boxed--left-block -->';
            } else if ( ( $current_post % 6 ) === 2 ) {
                echo '</div><!-- .grid-boxed--main-block -->';
            } else if ( ( $current_post % 6 ) === 5 ) {
                echo '</div><!-- .grid-boxed--right-block -->';
            } else if( $total_posts === ( $current_post + 1 ) ) {
                echo '</div><!-- .grid-boxed--end-wrapper/.grid-boxed--left-block/.grid-boxed--main-block/.grid-boxed--right-block -->';
            }
            endwhile;
            wp_reset_postdata();
