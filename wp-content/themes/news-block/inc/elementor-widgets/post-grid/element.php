 ...
 * 
 */
class News_Block_Post_Grid_Element extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'Post Grid', 'news-block' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'default'   => esc_html__( 'News Block Post Grid Title', 'news-block' ),
        ...
        $this->end_controls_section(); // color section ends
    }
    ...
    protected function render() {
        ...
        $paged = 1;
        if( $paginationOption ) {
            if( isset( $_GET[ 'gridpaged'.$blazemagemodulespaged ] ) ) {
                $paged = absint( $_GET[ 'gridpaged'.$blazemagemodulespaged ] );
            }
        }
        
        echo '<div id="blaze-mag-modules-post-grid-block-'.esc_attr( $element_id ).'" class="blaze-mag-modules-post-grid-block block-'.esc_attr( $element_id ).' bmm-block bmm-block-post-grid--'.esc_html( $blockLayout ).' bmm-block-image-hover--'.esc_html( $imageHoverType ).'">';
            if( !empty( $blockTitle ) ) {
                echo '<h2 class="bmm-block-title layout--default">'.esc_html( $blockTitle ).'</h2>';
            }
            // include template file w.r.t  layout value
            include( NEWS_BLOCKS_INCLUDES_PATH .'elementor-widgets/post-grid/'.$blockLayout.'/'.$blockLayout.'.php' );

            if( $paginationOption ) {
                $paginate_args = array(
                            'format' => '?gridpaged'.$blazemagemodulespaged.'=%#%',
                            'current' => max( 1, $paged ),
                            'total' => $max_num_pages,
                            'type' => 'gridpaged'
                        );
        ...
        echo '</div><!-- #blaze-mag-modules-post-grid-block -->';
    }
}